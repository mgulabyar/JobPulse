...
KEYWORD_SYNONYMS: Dict[str, List[str]] = {
    "excel addin": ["excel add-in", "excel add in", "office add-in", "office.js", "office js", "vsto", "excel plugin", "spreadsheet add-in"],
    "outlook addin": ["outlook add-in", "outlook add in", "office add-in", "office.js", "office js", "vsto"],
    "word addin": ["word add-in", "word add in", "office add-in", "office.js", "office js", "vsto"],
    "powerpoint addin": ["powerpoint add-in", "powerpoint add in", "office add-in", "office.js", "office js", "vsto"],
    "office addin": ["office add-in", "office.js", "office js", "vsto", "microsoft appsource", "spreadsheet add-in", "add-in developer"],
    "office.js": ["office add-in", "office js", "excel add-in", "outlook add-in", "word add-in", "powerpoint add-in"],
    "google workspace": ["google workspace add-on", "apps script", "google apps script", "gsuite", "workspace addon", "google sheets add-on", "google docs add-on", "gmail add-on"],
    "google sheets addon": ["sheets add-on", "apps script", "google apps script", "google sheets api"],
    "google docs addon": ["docs add-on", "apps script", "google apps script"],
    "gmail addon": ["gmail add-on", "apps script", "google apps script", "gmail api"],
    "apps script": ["google apps script", "google workspace", "gas developer", "workspace add-on"],
    "mern": ["mongodb", "express", "react", "node.js", "nodejs", "full stack javascript"],
    "asp.net core": ["asp.net", "dotnet core", ".net core", "c# backend"],
    "next.js": ["nextjs", "react", "vercel", "ssr framework"],
    "python": ["django", "flask", "fastapi", "python backend"],
    "full stack": ["frontend", "backend", "full-stack developer", "mern", "mean", "web developer"],
}

REMOTE_BOOST = [
    "remote", "work from home", "work remotely", "telecommute", "distributed", "worldwide", "anywhere"
]
FREELANCE_BOOST = [
    "freelance", "contract", "contractor", "project", "part-time", "part time", "consultant", "1099"
]
NEGATIVE_TERMS = [
    "assistant", "admin", "sales", "marketing", "support", "customer service", "recruiter", "hr", "human resources", "intern", "data entry"
]
...
def relevance_score(title: str, desc: str, company: str, search_terms: List[str]) -> int:
    ...
    if any(x in text for x in REMOTE_BOOST):
        score += 4
    if any(x in text for x in FREELANCE_BOOST):
        score += 4
...
def safe_get_json(url: str, timeout: int = 12) -> Any:
...
def fetch_remoteok(keyword: str) -> List[Dict]:
    jobs = []
    try:
        data = safe_get_json("https://remoteok.com/api")
        if isinstance(data, list):
            ...
            for job in data:
                ...
                company = job.get("company", "")
                blob = f"{title} {desc} {tags_text} {company}".lower()
                ...
                jobs.append({
                    "title": title,
                    "company": company,
                    "apply_link": job.get("url") or job.get("apply_url") or "#",
                    "raw_desc": desc + " " + tags_text,
                    "source": "RemoteOK",
                })
    except Exception as e:
        print("[REMOTEOK ERROR]", repr(e))
    return jobs


def fetch_wwr(keyword: str) -> List[Dict]:
    jobs = []
    try:
        url = "https://weworkremotely.com/categories/remote-programming-jobs.rss"
        resp = requests.get(url, headers=HEADERS, timeout=12)
...
def fetch_arbeitnow(keyword: str) -> List[Dict]:
    jobs = []
    try:
        data = safe_get_json("https://www.arbeitnow.com/api/job-board-api")
        if isinstance(data, dict):
            kw = keyword.lower().strip()
            for job in data.get("data", []):
                title = job.get("title", "Job Position")
...
def fetch_jobicy(keyword: str) -> List[Dict]:
    ...
    try:
        ...
        print("[JOBICY ERROR]", repr(e))
    return jobs


def normalize_jobs(all_jobs: List[Dict], search_terms: List[str]) -> List[Dict]:
    seen_links = set()
    scored = []

    for job in all_jobs:
        link = job.get("apply_link", "#")
        if not link or link == "#" or link in seen_links:
            continue
        seen_links.add(link)
...
@app.get("/fetch-jobs")
def fetch_jobs(keyword: str = Query(..., description="Job keyword, e.g. 'Office Add-in', 'Google Apps Script', 'MERN'")):
    print("\n================ JOB SEARCH START ================")
    ...
    all_jobs.extend(fetch_wwr(keyword))
    all_jobs.extend(fetch_arbeitnow(keyword))
    all_jobs.extend(fetch_jobicy(keyword))
    ...
    if not all_jobs:
        return {
            "status": "error",
            "message": "No data received from sources. Check console logs.",
            "debug": "All sources returned empty or failed"
        }
    ...
    if not top_jobs:
        return {
            "status": "error",
            "message": f"'{keyword}' ke liye abhi strong remote/freelance match nahi mili. Better keywords try karein.",
            "debug": {
    ...
    for job in top_jobs:
        ...
        if job.get("company"):
            title = f"{title} - {job['company']}"
        desc = job["raw_desc"][:400] if job["raw_desc"] else f"Live posting from {job['source']}."
        jobs_list.append({
            "title": title,
            "description": desc,
            "apply_link": job["apply_link"],
...
def home_page():
    ...
    <head>
        ...
        <title>JobPulse - Freelance Developer Job Finder</title>
        <style>
            body { font-family: Arial, sans-serif; text-align: center; margin: 0; padding: 40px; background: #f8f9fa; }
            h2 { color: #212529; margin-bottom: 5px; }
            p.subtitle { color: #6c757d; margin-bottom: 20px; font-size: 14px; }
            .chips { margin-bottom: 20px; }
            .chip { display: inline-block; background: #e7f0ff; color: #0056b3; border: 1px solid #b6d4fe; padding: 6px 14px; border-radius: 20px; margin: 4px; font-size: 13px; cursor: pointer; }
            .chip:hover { background: #d0e2ff; }
            .search-container { margin-bottom: 30px; }
            input { padding: 12px 20px; width: 420px; border: 2px solid #dee2e6; border-radius: 25px; font-size: 16px; outline: none; }
            input:focus { border-color: #0056b3; box-shadow: 0 0 8px rgba(0,86,179,0.2); }
            button { padding: 12px 25px; background: #0056b3; color: white; border: none; border-radius: 25px; cursor: pointer; font-size: 16px; font-weight: bold; margin-left: 10px; }
            button:hover { background: #004085; }
            #results { margin: 0 auto; text-align: left; width: 100%; max-width: 850px; }
            .job-card { background: white; padding: 20px; margin-bottom: 15px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border-left: 5px solid #0056b3; }
            .job-card a { color: #0056b3; text-decoration: none; font-weight: bold; font-size: 18px; }
            .job-card a:hover { text-decoration: underline; }
            .meta { margin: 5px 0; color: #198754; font-size: 13px; font-weight: bold; }
            .desc { color: #495057; font-size: 14px; line-height: 1.5; margin-top: 8px; }
            .loading { font-size: 16px; color: #6c757d; font-weight: 500; }
            .error-box { background: #fff5f5; color: #cc0000; border: 1px solid #ffcccc; padding: 15px; border-radius: 8px; margin-top: 20px; }
        </style>
    ...
    <body>
        <h2>JobPulse</h2>
        <p class="subtitle">Freelance, Remote & Developer Jobs for Office Add-ins, Apps Script, MERN, Next.js, Python</p>

        <div class="chips">
            <span class="chip" onclick="quickSearch('Office Add-in developer')">Office Add-in</span>
            <span class="chip" onclick="quickSearch('Excel add-in developer')">Excel Add-in</span>
            <span class="chip" onclick="quickSearch('Google Apps Script developer')">Apps Script</span>
            <span class="chip" onclick="quickSearch('Google Workspace add-on developer')">Workspace</span>
            <span class="chip" onclick="quickSearch('MERN stack developer remote')">MERN</span>
            <span class="chip" onclick="quickSearch('Next.js freelance developer')">Next.js</span>
            <span class="chip" onclick="quickSearch('Python backend contract')">Python</span>
        </div>

        <div class="search-container">
            <input type="text" id="keyword" placeholder="e.g. Office Add-in developer, Google Apps Script, MERN, freelance Next.js">
            <button onclick="searchJobs()">Search Jobs</button>
        </div>
        ...
        <script>
            ...
            async function searchJobs() {
                ...
                let div = document.getElementById('results');
                if(!kw || kw.trim() === "") {
                    alert("Meharbani karke koi keyword likhein!");
                    return;
                }

                div.innerHTML = "<p class='loading'>Relevant remote/freelance developer jobs dhoondi ja rahi hain...</p>";
                ...
                try {
                    ...
                    if(data.status === 'success' && data.jobs.length > 0) {
                        ...
                        data.jobs.forEach(job => {
                            div.innerHTML += `
                                <div class="job-card">
                                    <a href="${job.apply_link}" target="_blank" rel="noopener noreferrer">${job.title}</a>
                                    <p class="meta">${job.source} | Score: ${job.score}</p>
                                    <p class="desc">${job.description}</p>
                                </div>`;
                        });
                    } else {
                        div.innerHTML = `<div class="error-box">${data.message || 'No jobs found'}</div>`;
                    }
                } catch(err) {
                    console.error(err);
                    div.innerHTML = "<div class='error-box'>System processing error. Browser refresh karke try karein.</div>";
